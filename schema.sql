...
create table user_standup_stats (
...
create or replace function update_user_standup_stats()
returns trigger
language plpgsql
as $$
declare
...
begin
  ...
  if prev_date is null then
    insert into user_standup_stats (
      user_id,
      org_id,
      total_standups,
      current_streak,
...
for each row
execute function update_user_standup_stats();

create view team_daily_stats as
select
  org_id,
  standup_date,
